...
global choice_num
choice_num = sys.argv[2]

save_path = '../output_features/'+str(sys.argv[3])+'/features_out/' #features 
test_input_image = '../data/testset_diffnoise/0/'
png_index = []
load_rendering_fv = []
...
def run():
    ...
    data_num = 60
    log = open("result"+str(choice_num)+str(".txt"), "w")
    ...
    for j in range(file_num):
        ...
        #print("des= ", int(str(files[j])[0:2]))
        #print("estimated= ",estimated_th1)
        des.append(int(str(files[j])[0:2]))   
        est.append(estimated_th1) 

        log.write('%s %f' % (str(files[j])[0:2], estimated_th1))
        log.write('\n')
...
def main(_):
    ...
    load_init_fv()
    init_kd_tree(load_rendering_fv, 60, 0)
