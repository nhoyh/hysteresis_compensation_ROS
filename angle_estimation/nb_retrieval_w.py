...
case = sys.argv[2]
save_path = './features_out/' 
test_input_image = './data/resize_128/real/' + str(case) + str('/')
png_index = []
load_rendering_fv = []
...
def run():
    ...
    for j in range(file_num):
        ...
        rendering = cv2.resize(rendering, dsize=(128, 128))
        ...
        log.write('%f %f' % (int(str(files[j])[0:2]), estimated_th1))
        log.write('\n')
...
def main(_):
    ...
    load_init_fv()
    init_kd_tree(load_rendering_fv, 61, 0)
